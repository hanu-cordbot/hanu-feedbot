#!/usr/bin/env python3
"""
Enhanced cron worker that uses the improved main module with parallel processing
"""
import asyncio
import sys
import os
...
async def main():
    """Main entry point with enhanced bot functionality"""
    start_time = time.time()
    lock_fd = acquire_lock()
    
    try:
        print(f"Starting enhanced bot job at {time.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        # Set up a timeout for the entire job (10 minutes)
        ...
        try:
            # Import the enhanced bot main module
            print("Importing enhanced bot modules...")
            from bot.main_enhanced import run_bot_job
            
            # Run the enhanced bot job with timeout
            ...
            if bot_task in done:
                ...
                print(f"Enhanced bot job completed successfully in {duration:.2f} seconds")
            else:
        ...
        await asyncio.sleep(3)  # Reduced to 3 seconds
        
        duration = time.time() - start_time
        print(f"Enhanced job finished successfully in {duration:.2f} seconds")
        return 0
...
if __name__ == "__main__":
    print("Starting enhanced cron worker...")
    try:
        exit_code = asyncio.run(main())
        print(f"Enhanced cron worker finished with exit code {exit_code}")
        sys.exit(exit_code)
